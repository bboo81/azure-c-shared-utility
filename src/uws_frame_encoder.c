...
int uws_frame_encoder_encode(BUFFER_HANDLE encode_buffer, WS_FRAME_TYPE opcode, const void* payload, size_t length, bool is_masked, bool is_final, unsigned char reserved)
{
    ...
    else if (opcode > 0x0F)
    {
        ...
        (payload == NULL))
    {
        /* Codes_SRS_UWS_FRAME_ENCODER_01_054: [ If `length` is greater than 0 and payload is NULL, then `uws_frame_encoder_encode` shall fail and return a non-zero value. ]*/
